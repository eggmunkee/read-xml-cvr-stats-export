...
public class CVRParse {
    ...
    // Call callback with column name and string value, return item for additional processing
    public static XElement? CallIfHasStringValueRow(StatUpdateRowString callback, XElement? item)
    {
        if (item != null) {
            callback(item.Name.LocalName, item.Value);
        }
    ...
    public delegate void StatUpdateRowString(string columnName, string value);
    public delegate void StatUpdateRowInt(string columnName, int value);
}